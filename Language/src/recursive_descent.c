...
static const token *p_tokens = NULL;
...
static Node *print_error(const char *waited) {
  fprintf(stderr, "Mistake at line %zu: waited \"%s\", in fact ",
...
#define KEYWORD(name, keyword, ident) else if (keyword == type) return ident;
...
static Node *GetGrammar() {
  ...
  while (!is_keyword(iterator, KEY_STOP) &&
         (is_ident(iterator) || is_keyword(iterator, KEY_CONST))) {
  ...
  if (is_keyword(iterator, KEY_CONST) ||
      (is_ident(iterator) && is_keyword(iterator + 1, KEY_ASSIGN)) ||
...
static Node *GetFuncDef() {
  ...
  if (is_ident(iterator) && is_keyword(iterator + 1, KEY_OPEN)) {
    Node *new_node = create_oper(DEF, create_oper(FUNC, NULL, NULL), NULL);
    new_node->Left->Left = create_ident(is_ident(iterator), NULL, NULL);
    ...
    if (is_keyword(iterator, KEY_CLOSE)) {
      ...
      Node *param_node = create_oper(
          PARAM, NULL, create_ident(is_ident(iterator), NULL, NULL));
      new_node->Left->Right = param_node;

      iterator++;

      while (is_keyword(iterator, KEY_COMMA) && is_ident(iterator + 1) &&
             !is_keyword(iterator + 1, KEY_STOP)) {
        param_node = create_oper(
            PARAM, NULL, create_ident(is_ident(iterator + 1), NULL, NULL));
...
static Node *GetAssign() {
  ...
  if (is_ident(iterator)) {
    if (is_keyword(iterator + 1, KEY_ASSIGN)) {
      ...
      Node *new_node = NULL;
      if (is_const)
        new_node = create_oper(
            STATEMENT, NULL,
            create_oper(ASSIGN,
                        create_ident(is_ident(iterator - 2),
                                     create_oper(CONST, NULL, NULL), NULL),
                        GetExpression()));
      else
        new_node = create_oper(
            STATEMENT, NULL,
            create_oper(ASSIGN,
                        create_ident(is_ident(iterator - 2), NULL, NULL),
                        GetExpression()));
      ass(new_node->Right->Right);

      node = new_node;
    } else if ((bool)is_keyword(iterator + 1, KEY_QOPEN) &&
               (bool)is_number(iterator + 2) &&
               (bool)is_keyword(iterator + 3, KEY_QCLOSE) &&
               (bool)is_keyword(iterator + 4, KEY_ASSIGN) &&
               (bool)is_keyword(iterator + 5, KEY_BEGIN) &&
               (bool)is_number(iterator + 6)) {
      ...
      if (is_const)
        new_node = create_oper(
            STATEMENT, NULL,
            create_oper(ASSIGN,
                        create_ident(mas_ident, create_oper(CONST, NULL, NULL),
                                     create_num((double)mas_index, NULL, NULL)),
                        create_num(is_number(iterator - 1), NULL, NULL)));
      else
        new_node = create_oper(
            STATEMENT, NULL,
            create_oper(ASSIGN,
                        create_ident(mas_ident, NULL,
                                     create_num((double)mas_index, NULL, NULL)),
                        create_num(is_number(iterator - 1), NULL, NULL)));
      node = new_node;
      ...
      if (is_keyword(iterator, KEY_END)) {
        ...
        while ((bool)is_keyword(iterator, KEY_COMMA) &&
               (bool)is_number(iterator + 1) &&
               !(bool)is_keyword(iterator + 1, KEY_STOP)) {
          if (is_const)
            new_node->Left = create_oper(
                STATEMENT, NULL,
                create_oper(
      ...
      if (is_const)
        new_node =
            create_oper(STATEMENT, NULL,
                        create_oper(ASSIGN,
                                    create_ident(is_ident(iterator - 2),
                                                 create_oper(CONST, NULL, NULL),
                                                 GetExpression()),
                                    NULL));
      else
        new_node = create_oper(STATEMENT, NULL,
                               create_oper(ASSIGN,
                                           create_ident(is_ident(iterator - 2),
                                                        NULL, GetExpression()),
                                           NULL));
      ...
      if (!is_keyword(iterator, KEY_QCLOSE) ||
          !is_keyword(iterator + 1, KEY_ASSIGN))
        return print_error("] =");
...
static Node *GetFunction() {
  ...
  if (is_ident(iterator) && is_keyword(iterator + 1, KEY_OPEN)) {
    Node *new_node =
        create_oper(CALL, create_ident(is_ident(iterator), NULL, NULL), NULL);
    ...
    if (is_keyword(iterator, KEY_CLOSE)) {
      ...
      Node *param_node = create_oper(PARAM, NULL, GetExpression());
      ass(param_node->Right);
      new_node->Right = param_node;

      while (is_keyword(iterator, KEY_COMMA) &&
             !is_keyword(iterator, KEY_STOP)) {
        iterator++;

        param_node = create_oper(PARAM, NULL, GetExpression());
        ass(param_node->Right);
...
static Node *GetBlock() {
  ...
  if (is_keyword(iterator, KEY_BEGIN)) {
    ...
    do {
      ...
      if (is_keyword(iterator - 1, KEY_SEMICOL) &&
          is_keyword(iterator - 2, KEY_END)) {
...
static Node *GetOperation() {
  ...
  if (is_keyword(iterator, KEY_RETURN)) {
    ...
    Node *new_node = create_oper(RET, NULL, GetExpression());
    if (!new_node->Right)
    ...
    node = create_oper(STATEMENT, NULL, new_node);
  } else if (is_keyword(iterator, KEY_IF) &&
             is_keyword(iterator + 1, KEY_OPEN)) {
    iterator += 2;

    Node *new_node =
        create_oper(IF, GetExpression(), create_oper(DECISION, NULL, NULL));
    ass(new_node->Left);
    ...
    node = create_oper(STATEMENT, NULL, new_node);
  } else if (is_keyword(iterator, KEY_WHILE) &&
             is_keyword(iterator + 1, KEY_OPEN)) {
    iterator += 2;

    Node *new_node = create_oper(WHILE, GetExpression(), NULL);
    ass(new_node->Left);
    ...
    node = create_oper(STATEMENT, NULL, new_node);
  } else if (is_keyword(iterator, KEY_OUT) &&
             is_keyword(iterator + 1, KEY_OPEN)) {
    iterator += 2;

    Node *new_node = create_oper(PRINT, NULL, GetExpression());
    ass(new_node->Right);

    if (!is_keyword(iterator, KEY_CLOSE) ||
        !is_keyword(iterator + 1, KEY_SEMICOL)) {
      return print_error(");");
    }
    ...
    node = create_oper(STATEMENT, NULL, GetFunction());
    ass(node->Right);
    ...
    iterator++;
  } else if ((is_ident(iterator) && (is_keyword(iterator + 1, KEY_ASSIGN) ||
                                     is_keyword(iterator + 1, KEY_QOPEN))) ||
             is_keyword(iterator, KEY_CONST)) {
    node = GetAssign();
    ass(node);
  } else {
...
static Node *GetExpression() {
  ...
  if (is_keyword(iterator, KEY_NOT)) {
    ...
    node = create_oper(NOT, NULL, GetCompare());
    ass(node->Right);
  } else {
    node = GetCompare();
    ass(node);
  ...
  while ((is_keyword(iterator, KEY_OR) || is_keyword(iterator, KEY_AND)) &&
         !is_keyword(iterator, KEY_STOP)) {
    iterator++;
    ...
    if (is_keyword(iterator, KEY_NOT)) {
      ...
      if (is_keyword(iterator - 2, KEY_OR))
        new_node = create_oper(OR, node, create_oper(NOT, NULL, GetCompare()));
      else if (is_keyword(iterator - 2, KEY_AND))
        new_node = create_oper(AND, node, create_oper(NOT, NULL, GetCompare()));
      ass(new_node->Right->Right);
    } else {
...
static Node *GetCompare() {
  ...
  if (is_keyword(iterator, KEY_GREAT) || is_keyword(iterator, KEY_LOW) ||
      is_keyword(iterator, KEY_GEQUAL) || is_keyword(iterator, KEY_LEQUAL) ||
      is_keyword(iterator, KEY_EQUAL) || is_keyword(iterator, KEY_NEQUAL)) {
    iterator++;
...
static Node *GetBoolean() {
  ...
  while ((is_keyword(iterator, KEY_ADD) || is_keyword(iterator, KEY_SUB)) &&
         !is_keyword(iterator, KEY_STOP)) {
    iterator++;
...
static Node *GetT() {
  ...
  while ((is_keyword(iterator, KEY_MUL) || is_keyword(iterator, KEY_DIV)) &&
         !is_keyword(iterator, KEY_STOP)) {
    iterator++;
...
static Node *GetU() {
  ...
  if (is_keyword(iterator, KEY_OPEN)) {
    ...
    iterator++;
  } else if (is_keyword(iterator, KEY_SIN) &&
             is_keyword(iterator + 1, KEY_OPEN)) {
    iterator += 2;

    node = create_oper(SIN, NULL, GetExpression());
    ass(node->Right);
    ...
    iterator++;
  } else if (is_keyword(iterator, KEY_COS) &&
             is_keyword(iterator + 1, KEY_OPEN)) {
    iterator += 2;

    node = create_oper(COS, NULL, GetExpression());
    ass(node);
    ...
    iterator++;
  } else if (is_keyword(iterator, KEY_INT) &&
             is_keyword(iterator + 1, KEY_OPEN)) {
    iterator += 2;

    node = create_oper(INT, NULL, GetExpression());
    ass(node);
    ...
    iterator++;
  } else if (is_keyword(iterator, KEY_IN) &&
             is_keyword(iterator + 1, KEY_OPEN)) {
    iterator += 2;

    node = create_oper(SCAN, NULL, NULL);
    ...
    node = create_ident(is_ident(iterator - 2), NULL, GetExpression());
    ass(node);
    ...
    node = create_num(is_number(iterator), NULL, NULL);
    iterator++;
  } else if (is_ident(iterator) && !is_keyword(iterator + 1, KEY_OPEN)) {
    node = create_ident(is_ident(iterator), NULL, NULL);
    iterator++;
  } else if (is_ident(iterator) && is_keyword(iterator + 1, KEY_OPEN)) {
    node = GetFunction();
  } else {
