...
static int logfile_print(FILE **logfile, CPU *const p_cpu,
                         const Command *const p_current_command);

static int read_commands(const char *const binfile_path,
                         Command **const pp_commands,
                         size_t *const p_commands_number);
static int run_commands(FILE **logfile, CPU *const p_cpu,
                        Command *const p_commands,
                        const size_t commands_number);
...
static void CpuCtor(CPU *const p_cpu) {
  ...
  p_cpu->DarkRegisters =
      (double *)calloc(DARK_REGISTERS_NUMBER, sizeof(double));
...
static int logfile_init(FILE **logfile) {
  ...
  *logfile = fopen("logs/cpu_dump.txt", "w");
  if (!*logfile)
    return 0;
  ...
  fprintf(*logfile,
          ...
          "===================================================================="
          "==================\n"
          "|Name_cmd:   |Argument_type:  |Argument:   |Stack size: |Last 16 "
          "stack elements:                                                     "
          "                     \n"
          "|            |                |            |            |Registers: "
          "                                                                    "
...
static int logfile_print(FILE **logfile, CPU *const p_cpu,
  ...
  case is_char:
    fprintf(*logfile, "|CHAR            |%-5c(%-5lg)|",
            (char)p_current_command->argument, p_current_command->argument);
    break;
  case is_number:
    fprintf(*logfile, "|NUMBER          |%-12lg|", p_current_command->argument);
    break;
  case is_register:
    fprintf(*logfile, "|REGISTER        |%-5s(%-5lg)|",
            get_reg_string((char)p_current_command->argument),
            p_current_command->argument);
    break;
  case is_ram_register:
    fprintf(*logfile, "|RAM             | [%-4lg]     |",
            p_current_command->argument);
    break;
  case is_nothing:
