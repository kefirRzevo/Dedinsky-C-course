...
static int make_label(array *tokens, array *idents, char *label,
                      size_t label_size);
...
static int make_label(array *tokens, array *labels, char *label,
                      size_t label_size) {
  assert(tokens);
  ...
  for (size_t i = 0; i < labels->size; i++) {
    if (!strncmp(label, labels_data[i], label_size) &&
        label_size == strlen(labels_data[i])) {
...
static void find_ending(size_t *token_length) {
  ...
  while (!isspace(*iterator) && *iterator != ';' && !is_key(*iterator) &&
         *iterator != '\0')
    iterator++;
...
static int tokens_dump(const array *const tokens, const char *dump_file_path) {
  ...
  for (size_t i = 0; i < tokens->size; i++) {
    ...
    case CMD:
      fprintf(fp,
              "|%-4zu|LINE: %-4zu|COMMAND   |VALUE:   %-15s|\n"
              "|====|==========|==========|========================|\n",
              i + 1, temp_token.line, get_cmd_string(temp_token.value.command));
      break;
    case REG:
      fprintf(fp,
              "|%-4zu|LINE: %-4zu|REGISTER  |VALUE:   %-15s|\n"
              "|====|==========|==========|========================|\n",
    ...
    case KW:
      fprintf(fp,
              "|%-4zu|LINE: %-4zu|KEYWORD   |VALUE:   %-15c|\n"
              "|====|==========|==========|========================|\n",
              i + 1, temp_token.line, temp_token.value.keyword);
      break;
    case CH:
      fprintf(fp,
              "|%-4zu|LINE: %-4zu|CHAR      |VALUE:   %-15c|\n"
              "|====|==========|==========|========================|\n",
              i + 1, temp_token.line, temp_token.value.ch);
      break;
    case LAB:
      fprintf(fp,
              "|%-4zu|LINE: %-4zu|LABEL     |VALUE:   %-15s|\n"
              "|====|==========|==========|========================|\n",
