...
static size_t get_file_size(FILE **const pp_file, char *textfile_path,
                            char **pp_buf);
...
static Node *GetE() {
  ...
  while (check_bin_opers(&oper, OPER_ADD, OPER_SUB)) {
    iterator++;
    Node *oper_node = new_oper_node((OperType)oper, NULL, NULL);
...
static Node *GetU() {
  ...
  if (*iterator == '(') {
    ...
    iterator += 2;
    new_node = new_oper_node(OPER_DDX, NULL, NULL);
    new_node->Left = GetE();
    ...
    iterator++;
    new_node = new_oper_node((OperType)oper, NULL, NULL);
    new_node->Right = GetE();
    if (*iterator != ')') {
      fprintf(stderr, "No closing bracket : %s\n", iterator);
      node_delete(new_node);
