...
static Node *oper_dif(Node *n) {
  ...
  case VAR:
    if (n->Value.variable == variable)
      return NUM(1);
    else
      return NUM(0);

  case OPER:
    switch (n->Value.oper_type) {
    case OPER_ADD:
      return ADD(D(L), D(R));
    case OPER_SUB:
      return SUB(D(L), D(R));
    ...
    case OPER_DIV:
      return DIV(SUB(MUL(D(L), C(R)), MUL(C(L), D(R))),
                 OPER(OPER_POW, C(R), NUM(2)));
    case OPER_POW:
      switch (variable_location(n, variable)) {
      case OUT:
        return NUM(0);
      case LEFT:
        return MUL(D(L), MUL(C(R), OPER(OPER_POW, C(L), SUB(C(R), NUM(1)))));
      case RIGHT:
        return MUL(D(R),
                   MUL(OPER(OPER_LN, NULL, C(L)), OPER(OPER_POW, C(L), C(R))));
      case BOTH:
        return MUL(OPER(OPER_POW, C(L), C(R)),
                   ADD(MUL(D(R), OPER(OPER_LN, NULL, C(L))),
                       DIV(MUL(C(R), D(L)), C(L))));
      }
    case OPER_LN:
      return DIV(D(R), C(R));
    case OPER_SIN:
      return MUL(D(R), OPER(OPER_COS, NULL, C(R)));
    case OPER_COS:
      return MUL(NUM(-1), MUL(D(R), OPER(OPER_SIN, NULL, C(R))));
    case OPER_SH:
      return MUL(D(R), OPER(OPER_CH, NULL, C(R)));
    case OPER_CH:
      return MUL(D(R), OPER(OPER_SH, NULL, C(R)));
    case OPER_DDX:
      return NULL;
    }
  default:
