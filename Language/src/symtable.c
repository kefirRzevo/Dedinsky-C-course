...
static FILE *sym_table_dump_init();
...
array *create_scoop_level(sym_table *table) {
  ...
  if (!array_push(&table->scoop_levels, &temp_arr))
    return NULL;
  return (array *)((char *)table->scoop_levels.data +
                   (table->scoop_levels.size - 1) * sizeof(array));
}
...
func *add_function(sym_table *table, Node *func_node) {
  ...
  if (!is_node_opertor(func_node, FUNC))
    return NULL;

  if (!is_node_ident(func_node->Left))
    return NULL;

  func temp_func = {};

  temp_func.name = is_node_ident(func_node->Left);
  ...
  return (func *)((char *)table->functions.data +
                  (table->functions.size - 1) * sizeof(func));
}
...
var *add_variable(sym_table *table, Node *var_node) {
  ...
  if (!is_node_ident(var_node))
    return NULL;
  var temp_var = {};
  temp_var.adress = table->shift;
  temp_var.name = is_node_ident(var_node);
  temp_var.declared = var_node;
  temp_var.is_const = false;
  temp_var.size = 1;
  if (is_node_opertor(var_node->Left, CONST))
    temp_var.is_const = true;
  if (is_node_natural(var_node->Right))
    temp_var.size = get_node_number(var_node->Right) + 1;
  else if (var_node->Right)
    return NULL;
  table->shift += temp_var.size;
  array temp_arr = {};
  memcpy(&temp_arr,
         (char *)table->scoop_levels.data +
             (table->scoop_levels.size - 1) * sizeof(array),
         sizeof(array));
  if (!array_push(&temp_arr, &temp_var))
    return NULL;

  memcpy((char *)table->scoop_levels.data +
             (table->scoop_levels.size - 1) * sizeof(array),
         &temp_arr, sizeof(array));
...
func *find_function(sym_table *table, Node *func_node) {
  ...
  if (!func_node)
    return NULL;
  if (!is_node_opertor(func_node, CALL))
    return NULL;
  if (!is_node_ident(func_node->Left))
    return NULL;
  for (size_t i = 0; i < table->functions.size; i++) {
    ...
    memcpy(&temp_func, (char *)table->functions.data + i * sizeof(func),
           sizeof(func));
    if (!strcmp(temp_func.name, is_node_ident(func_node->Left))) {
      size_t n_params = 0;
      func_node = func_node->Right;

      while (func_node) {
        if (!is_node_opertor(func_node, PARAM))
          return NULL;
...
var *find_variable(sym_table *table, Node *var_node) {
  ...
  for (size_t i = 0; i < table->scoop_levels.size; i++) {
    array temp_arr = {};
    memcpy(&temp_arr, (char *)table->scoop_levels.data + i * sizeof(array),
           sizeof(array));
    ...
    for (size_t j = 0; j < temp_arr.size; j++) {
      ...
      if (!strcmp(temp_var.name, is_node_ident(var_node)))
...
static FILE *sym_table_dump_init() {
  ...
  LOGFILE = fopen("logs/symtable_dump.txt", "w");
  if (!LOGFILE)
    return NULL;
  char day[16] = "";
  switch (date->tm_wday) {
    ...
    for (size_t i = 0; i < table->scoop_levels.size; i++) {
      ...
      for (size_t j = 0; j < temp_arr.size; j++) {
        ...
        fprintf(LOGFILE,
                ...
                "=============================================================="
                "====\n",
                temp_var.name, temp_var.declared, temp_var.size,
                temp_var.adress, temp_var.is_const ? "true" : "false");
      }
    }
  }
  return table;
