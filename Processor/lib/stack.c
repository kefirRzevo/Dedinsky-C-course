...
static const size_t MAX_CAPACITY = 100000000;
...
static void check_stack(stack *const p_stack, const char *const stack_name,
                        Functions current_cmd, uint64_t error,
                        item_t *p_element, item_t element);
...
static void logfile_init() {
  ...
  LOGFILE = fopen("logs/stack_logfile.txt", "w");
  assert(LOGFILE);
...
static void realloc_stack(stack *const p_stack, size_t new_capacity) {
  ...
  // sizeof(canary_t)) = 0;
  const item_t *const data =
      (item_t *)((char *)p_stack->data - sizeof(canary_t));
...
static void check_stack(stack *const p_stack, const char *const stack_name,
  ...
  switch (current_cmd) {
  case ctor:
    strcpy(function, "stack_ctor");
    break;
  case dtor:
    strcpy(function, "stack_dtor");
    break;
  case push:
    strcpy(function, "stack_push");
    break;
  case pop:
    strcpy(function, "stack_pop");
    break;
  case dump:
    strcpy(function, "stack_dump");
    break;
  }
...
void stack_ctor(stack *const p_stack, const char *const stack_name) {
  ...
  if (p_stack->size != 0 || p_stack->capacity != 0 || p_stack->data != NULL) {
    ...
    check_stack(p_stack, stack_name, ctor, error, NULL, 0);
...
final_check:
  check_stack(p_stack, stack_name, dtor, error, NULL, 0);
}
...
void stack_push(stack *const p_stack, const char *const stack_name,
  ...
  uint64_t error = verify_stack(p_stack);
  if (error) {
    check_stack(p_stack, stack_name, push, error, NULL, element);
...
#endif
          ,
          stack_name, p_stack, p_stack->size, p_stack->capacity, p_stack->data
...
#ifdef StackCanaryProtect
          ,
          p_stack->LeftCanary, p_stack->RightCanary
#endif
  );
#ifdef StackCanaryProtect
  if (data != NULL)
    fprintf(LOGFILE,
            "| Data_left_canary:   %-30" CANARY_SPECIFICATOR "|\n"
