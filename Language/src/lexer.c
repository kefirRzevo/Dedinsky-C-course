...
static bool is_number(array *tokens, char *position, double *number,
                      size_t *num_len);
static bool push_ident(array *tokens, array *idents, char *position,
                       size_t id_len, size_t line);
...
static bool is_number(array *tokens, char *position, double *number,
  ...
  if (!(isdigit(*position) || (is_keyword_before(tokens) && *position == '-' &&
                               isdigit(*(position + 1)))))
    return false;
...
static bool push_ident(array *tokens, array *idents, char *position,
  ...
  for (size_t i = 0; i < idents->size; i++) {
    if (!strncmp(position, idents_data[i], id_len) &&
        id_len == strlen(idents_data[i])) {
...
static bool is_keyword_before(array *tokens) {
  ...
  return (
      (*((token *)((char *)tokens->data + (tokens->size - 1) * sizeof(token))))
          .type == KEYWORD);
}
...
token *tokenize(const char *file_path, array *idents, array *tokens) {
  ...
  tokens_dump(tokens, "logs/token_dump.txt");
...
static int tokens_dump(const array *const tokens, const char *dump_file_path) {
  ...
  array_dump_init(fp, tokens);
  fprintf(fp, "|====|==========|==========|===================================="
              "====================================\n");

  for (size_t i = 0; i < tokens->size; i++) {
    token temp_token =
        *((token *)((char *)tokens->data + i * tokens->item_size));
    switch (temp_token.type) {
    case KEYWORD:
      fprintf(fp,
              "|%-4zu|LINE: %-4zu|KEYWORD   |VALUE:   %-15s\n"
              "|====|==========|==========|===================================="
              "====================================\n",
    ...
    case NUMBER:
      fprintf(fp,
              "|%-4zu|LINE: %-4zu|NUMBER    |VALUE:   %-15lg\n"
              "|====|==========|==========|===================================="
              "====================================\n",
