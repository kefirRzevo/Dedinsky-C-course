...
#define SUCCESS                                                                \
  fprintf(LOG_FILE, "STACK ADDRESS: %8p %-9s SUCCESS                     \n",  \
          p_stack, __func__);                                                  \
  return SUCCESS

#define ERROR_STACK_IS_EMPTY                                                   \
  fprintf(LOG_FILE, "STACK ADDRESS: %8p %-9s ERROR_STACK_IS_EMPTY        \n",  \
          p_stack, __func__);                                                  \
  return ERROR_STACK_IS_EMPTY

#define ERROR_STACK_UNITIALIZED                                                \
  fprintf(LOG_FILE, "STACK ADDRESS: %8p %-9s ERROR_STACK_UNITIALIZED     \n",  \
          p_stack, __func__);                                                  \
  return ERROR_STACK_UNITIALIZED

#define ERROR_STACK_REINITIALIZATION                                           \
  fprintf(LOG_FILE, "STACK ADDRESS: %8p %-9s ERROR_STACK_REINITIALIZATION\n",  \
          p_stack, __func__);                                                  \
  return ERROR_STACK_REINITIALIZATION

#define ERROR_UNALLOCATED_MEMORY                                               \
  fprintf(LOG_FILE, "STACK ADDRESS: %8p %-9s ERROR_UNALLOCATED_MEMORY    \n",  \
          p_stack, __func__);                                                  \
...
#define SUCCESS                                                                \
  fprintf(LOG_FILE,                                                            \
          "STACK ADDRESS: %8p %-9s SUCCESS                      HASH: "        \
          "%-10" HASH_SPECIFICATOR "\n",                                       \
          p_stack, __func__, p_stack->hash);                                   \
  return SUCCESS

#define ERROR_STACK_IS_EMPTY                                                   \
  fprintf(LOG_FILE,                                                            \
          "STACK ADDRESS: %8p %-9s ERROR_STACK_IS_EMPTY         HASH: "        \
          "%-10" HASH_SPECIFICATOR "\n",                                       \
          p_stack, __func__, p_stack->hash);                                   \
  return ERROR_STACK_IS_EMPTY
...
#define ERROR_STACK_UNITIALIZED                                                \
  fprintf(LOG_FILE,                                                            \
          "STACK ADDRESS: %8p %-9s ERROR_STACK_UNITIALIZED      HASH: "        \
          "%-10" HASH_SPECIFICATOR "\n",                                       \
          p_stack, __func__, p_stack->hash);                                   \
  return ERROR_STACK_UNITIALIZED
...
#define ERROR_STACK_REINITIALIZATION                                           \
  fprintf(LOG_FILE,                                                            \
          "STACK ADDRESS: %8p %-9s ERROR_STACK_REINITIALIZATION HASH: "        \
          "%-10" HASH_SPECIFICATOR "\n",                                       \
          p_stack, __func__, p_stack->hash);                                   \
  return ERROR_STACK_REINITIALIZATION
...
#define ERROR_UNALLOCATED_MEMORY                                               \
  fprintf(LOG_FILE,                                                            \
          "STACK ADDRESS: %8p %-9s ERROR_UNALLOCATED_MEMORY     HASH: "        \
          "%-10" HASH_SPECIFICATOR "\n",                                       \
          p_stack, __func__, p_stack->hash);                                   \
  return ERROR_UNALLOCATED_MEMORY
#endif
...
#ifndef StackHashProtect
#define SUCCESS                                                                \
  const char *canary_result_check = "";                                        \
  Canary_Check(p_stack, &canary_result_check);                                 \
  fprintf(LOG_FILE,                                                            \
          "STACK ADDRESS: %8p %-9s SUCCESS                      %s\n",         \
...
#define ERROR_STACK_IS_EMPTY                                                   \
  ...
  Canary_Check(p_stack, &canary_result_check);                                 \
  fprintf(LOG_FILE,                                                            \
          "STACK ADDRESS: %8p %-9s ERROR_STACK_IS_EMPTY         %s\n",         \
          p_stack, __func__, canary_result_check);                             \
  return ERROR_STACK_IS_EMPTY
...
#define ERROR_STACK_UNITIALIZED                                                \
  ...
  Canary_Check(p_stack, &canary_result_check);                                 \
  fprintf(LOG_FILE,                                                            \
          "STACK ADDRESS: %8p %-9s ERROR_STACK_UNITIALIZED      %s\n",         \
          p_stack, __func__, canary_result_check);                             \
  return ERROR_STACK_UNITIALIZED
...
#define ERROR_STACK_REINITIALIZATION                                           \
  ...
  Canary_Check(p_stack, &canary_result_check);                                 \
  fprintf(LOG_FILE,                                                            \
          "STACK ADDRESS: %8p %-9s ERROR_STACK_REINITIALIZATION %s\n",         \
          p_stack, __func__, canary_result_check);                             \
  return ERROR_STACK_REINITIALIZATION
...
#define ERROR_UNALLOCATED_MEMORY                                               \
  ...
  Canary_Check(p_stack, &canary_result_check);                                 \
  fprintf(LOG_FILE,                                                            \
          "STACK ADDRESS: %8p %-9s ERROR_UNALLOCATED_MEMORY     %s\n",         \
          p_stack, __func__, canary_result_check);                             \
  return ERROR_UNALLOCATED_MEMORY
#endif
...
#define SUCCESS                                                                \
  ...
  Canary_Check(p_stack, &canary_result_check);                                 \
  fprintf(LOG_FILE,                                                            \
          "STACK ADDRESS: %8p %-9s SUCCESS                      %-30s      "   \
          "HASH: %-10" HASH_SPECIFICATOR "\n",                                 \
...
#define ERROR_STACK_IS_EMPTY                                                   \
  ...
  Canary_Check(p_stack, &canary_result_check);                                 \
  fprintf(LOG_FILE,                                                            \
          "STACK ADDRESS: %8p %-9s ERROR_STACK_IS_EMPTY         %-30s      "   \
          "HASH: %-10" HASH_SPECIFICATOR "\n",                                 \
          p_stack, __func__, canary_result_check, p_stack->hash);              \
  return ERROR_STACK_IS_EMPTY
...
#define ERROR_STACK_UNITIALIZED                                                \
  ...
  Canary_Check(p_stack, &canary_result_check);                                 \
  fprintf(LOG_FILE,                                                            \
          "STACK ADDRESS: %8p %-9s ERROR_STACK_UNITIALIZED      %-30s      "   \
          "HASH: %-10" HASH_SPECIFICATOR "\n",                                 \
          p_stack, __func__, canary_result_check, p_stack->hash);              \
  return ERROR_STACK_UNITIALIZED
...
#define ERROR_STACK_REINITIALIZATION                                           \
  ...
  Canary_Check(p_stack, &canary_result_check);                                 \
  fprintf(LOG_FILE,                                                            \
          "STACK ADDRESS: %8p %-9s ERROR_STACK_REINITIALIZATION %-30s      "   \
          "HASH: %-10" HASH_SPECIFICATOR "\n",                                 \
          p_stack, __func__, canary_result_check, p_stack->hash);              \
  return ERROR_STACK_REINITIALIZATION
...
#define ERROR_UNALLOCATED_MEMORY                                               \
  ...
  Canary_Check(p_stack, &canary_result_check);                                 \
  fprintf(LOG_FILE,                                                            \
          "STACK ADDRESS: %8p %-9s ERROR_UNALLOCATED_MEMORY     %-30s      "   \
          "HASH: %-10" HASH_SPECIFICATOR "\n",                                 \
          p_stack, __func__, canary_result_check, p_stack->hash);              \
  return ERROR_UNALLOCATED_MEMORY
#endif
...
#define open_log_file                                                          \
  if (STATUS_OPEN_LOG_FILE == false)                                           \
    LOG_FILE = fopen("logs/stack2_logfile.txt", "w");                          \
  STATUS_OPEN_LOG_FILE = true

static bool realloc_stack(stack *const p_stack, size_t capacity);
static Check Canary_Check(const stack *const p_stack,
                          const char **canary_result_check);
...
Check StackCtor(stack *const p_stack) {
  ...
  if (p_stack->size != 0 || p_stack->capacity != 0 || p_stack->data != NULL) {
  ...
  void *data = NULL;
  data = (void *)calloc(
      INITIAL_CAPACITY * sizeof(item_t) + 2 * sizeof(canary_t), 1);
#endif
  ...
  *(canary_t *)((char *)data + INITIAL_CAPACITY * sizeof(item_t) +
                sizeof(canary_t)) = DataRightCanary;
...
Check StackDump(const stack *const p_stack) {
  ...
  bool check_capacity =
      p_stack->capacity >= p_stack->size && p_stack->capacity > 0;
  ...
  canary_t DataRightCanary =
      *(canary_t *)((char *)data + p_stack->capacity * sizeof(item_t) +
                    sizeof(canary_t));
  ...
  fprintf(LOG_FILE,
...
#endif
          ,
          p_stack, check_adress ? OK : BAD, p_stack->size,
          check_size ? OK : BAD, p_stack->capacity, check_capacity ? OK : BAD,
          ...
          p_stack->RightCanary, check_right_canary ? OK : BAD, DataLeftCanary,
          check_data_left_canary ? OK : BAD, DataRightCanary,
          check_data_right_canary ? OK : BAD
#endif
  );
...
#ifndef StackCanaryProtect
  item_t *p_new_elements =
      (item_t *)realloc((void *)data, new_capacity * sizeof(item_t));
#endif

#ifdef StackCanaryProtect
  //*(canary_t *) ((char *)p_stack->data + p_stack->capacity * sizeof(item_t) +
  // sizeof(canary_t)) = 0;
  item_t *p_new_elements = (item_t *)realloc(
      (void *)data, new_capacity * sizeof(item_t) + 2 * sizeof(canary_t));
#endif
...
#ifdef StackCanaryProtect
  *(canary_t *)((char *)p_new_elements + new_capacity * sizeof(item_t) +
                sizeof(canary_t)) = STACK_CANARY;
...
static Check Canary_Check(const stack *const p_stack,
  ...
  if (p_stack->LeftCanary != STACK_CANARY ||
      p_stack->RightCanary != STACK_CANARY || DataLeftCanary != STACK_CANARY ||
...
static hash_t ResultHash(const stack *const p_stack) {
  hash_t data_hash = 0, stack_hash = 0;

#ifndef StackCanaryProtect
  stack_hash = QHash(p_stack, 2 * sizeof(size_t) + sizeof(item_t *));
#else
  stack_hash = QHash((item_t *)((char *)p_stack + sizeof(canary_t)),
                     2 * sizeof(int) + sizeof(item_t *));
#endif

#ifndef StackCanaryProtect
  data_hash = QHash(p_stack->data, p_stack->capacity * sizeof(item_t));
#else
  data_hash = QHash((item_t *)((char *)p_stack->data + sizeof(canary_t)),
                    p_stack->capacity * sizeof(item_t));
#endif
