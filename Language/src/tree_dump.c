...
static const char *LOG_PATH = "logs/dump";
...
static FILE *tree_dump_init(const char *dump_path);
...
static FILE *tree_dump_init(const char *const dump_path) {
  assert(dump_path);
...
#define OPER(name, keyword, ident)                                             \
  else if (keyword == node->Value.oper_type) return ident;
...
static void print_nodes(Node *node, FILE *const dotfile) {
  ...
  case NUM:
    fprintf(dotfile,
            "\t\tNode%p[label = \"%" NUM_SPEC
            "\"shape = oval,    color = yellow]\n",
            node, node->Value.number);
    break;
  case OPER:
    fprintf(dotfile,
            "\t\tNode%p[label = \"%" OPER_SPEC
            "\"shape = diamond, color = red]\n",
            node, oper_name(node));
    break;
  case ID:
    fprintf(dotfile,
            "\t\tNode%p[label = \"%" ID_SPEC
            "\"shape = octagon, color = blue]\n",
            node, node->Value.ident);
    break;
  default:
    fprintf(stderr, "Unknown error, %d.", __LINE__);
    break;
  }
...
static void write_nodes(Node *node, FILE *const file) {
  ...
  switch (node->Type) {
  case NUM:
    fprintf(file, "%" NUM_SPEC "", node->Value.number);
    break;
  case OPER:
    fprintf(file, "%" OPER_SPEC "", oper_name(node));
    break;
  case ID:
    fprintf(file, "'%" ID_SPEC "'", node->Value.ident);
    break;
  default:
    fprintf(stderr, "Unknown error, %d.", __LINE__);
    break;
...
static char *get_ident(char **iterator, array *idents) {
  ...
  *iterator += 2;
  ...
  for (size_t i = 0; i < idents->size; i++) {
    ...
    if (!strncmp(idents_data[i], *iterator - n_symbols - 1, n_symbols + 1) &&
        n_symbols + 1 == strlen(idents_data[i])) {
      (*iterator)++;
