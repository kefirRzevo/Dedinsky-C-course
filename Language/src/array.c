...
static const size_t MAX_CAPACITY = 1280;
...
int array_push(array *const p_array, void *const item) {
  ...
  memcpy((char *)p_array->data + p_array->item_size * p_array->size, item,
         p_array->item_size);
  p_array->size++;
  return 1;
}
...
int array_pop(array *const p_array, void *const item) {
  ...
  p_array->size--;
  memcpy(item, (char *)p_array->data + p_array->item_size * p_array->size,
         p_array->item_size);
  return 1;
}
...
void array_dump_init(FILE *fp, const array *const p_array) {
  ...
  fprintf(fp,
          ...
          "| Data:               %-78p\n"
          "| Item size:          %-78zu\n",
          date->tm_hour, date->tm_min, date->tm_sec, day, p_array,
          p_array->size, p_array->capa, p_array->data, p_array->item_size);
}
