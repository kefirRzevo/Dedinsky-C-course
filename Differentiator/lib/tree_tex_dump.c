...
FILE *LOGFILE = NULL;
const char LOGFILE_PATH[] = "logs/dump.tex";
...
FILE *tex_open() {
  if (!LOGFILE) {
    ...
    fprintf(LOGFILE,
            ...
            "\t\t\\vspace{0.5cm}                                        \n"
            "\t\t\\vspace{1.5cm}                                        \n"
            "\t\t\\includegraphics[width = 10 cm]{res/petrovich.png}    \n"
            "\t\t\\begin{minipage}{10cm}                                \n"
            "\t\t\\vspace*{2cm}                                         \n"
            "\t\t\t\\begin{center}                                      \n"
            "\t\t\t\tИз МФТИ, с любовью                                 \n"
            "\t\t\t\\end{center}                                        \n"
            "\t\t\\includegraphics[width = 3 cm]{res/with_love.png}     \n"
            "\t\t\\end{minipage}                                        \n"
            "                                                           \n"
            "\t\\end{center}                                            \n"
            "\\end{titlepage}                                           \n"
...
void tex_tree(Tree *p_tree, bool phrase) {
  ...
  if (CHANGE_STACK.size)
    tex_msg("Где, \n");

  while (CHANGE_STACK.size) {
    Node *temp = NULL;
    StackPop(&CHANGE_STACK, &temp);
...
static void tex_eq_start(bool phraze) {
  if (phraze)
    fprintf(LOGFILE, "%s ",
            CRINGE[rand() % (sizeof(CRINGE) / sizeof(*CRINGE))]);

  fprintf(LOGFILE, "\\begin{equation}\n\t");
}

static void tex_eq_end() { fprintf(LOGFILE, "\n\\end{equation}\n"); }

static void left_bracket() { fprintf(LOGFILE, "\\left( "); }

static void right_bracket() { fprintf(LOGFILE, "\\right) "); }
...
static void tex_data(Node *n) {
  ...
  switch (T) {
  case VAR:
    fprintf(LOGFILE, "%" VAR_SPEC "", V);
    break;
  case NUM:
    fprintf(LOGFILE, "%" NUM_SPEC "", N);
    break;
  default:
    break;
  }
...
static void tex_uno(Node *n) {
  ...
  case OPER_COS:
    ...
    break;
  case OPER_SH:
    fprintf(LOGFILE, "\\sinh {");
    tex_node(R);
    break;
  case OPER_CH:
    fprintf(LOGFILE, "\\cosh {");
    tex_node(R);
    break;
  default:
    return;
...
static void fill_substitutions(Node *n, Tree *p_tree) {
  ...
  if (size > 10 && size < 20 && p_tree->size > 30) {
    ...
    new_node->Type = VAR;
    new_node->Right = n;
    new_node->Value.variable = 'A' + CHANGE_STACK.size;

    StackPush(&CHANGE_STACK, new_node);
    return;
  }
...
static Node *find_node(Node *n) {
  ...
  Node *found = NULL;
  for (size_t i = CHANGE_STACK.size; i > 0; i--) {
    if ((CHANGE_STACK.data[i - 1])->Right == n) {
      found = CHANGE_STACK.data[i - 1];
      break;
    }
  }
